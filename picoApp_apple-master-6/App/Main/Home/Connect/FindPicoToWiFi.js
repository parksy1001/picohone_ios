...
import CustomModal from '../../../component/CustomModalBackSignIn';
...
export default class FindPico extends Component {
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.wifiView}>
          {(this.state.scanning == true) || (this.state.isScanned ==true)? (
        ...
        <View style={styles.buttonView}>
          <TouchableOpacity
            style={styles.button}
            onPress={() => this.getBLEPermission()}>
            <Text style={styles.buttonText}>{findText}</Text>
          </TouchableOpacity>
        </View>
        {/* Modal Hide */}
        <Modal
          isVisible={this.state.bleModal}
          onBackdropPress={() => this.setState({bleModal: false})}>
          <View>
            {this.scanLoading ? (
              ...
              <View style={styles.modalContainer}>
                <View style={styles.modalCancel}>
                  <TouchableOpacity
                    onPress={() => this.setState({bleModal: false})}>
                    <Image
  ...
  handleStopScan() {
    ...
    this.setState({scanning: false, scanEnd: true, isScanned: true});
    let picos = [];
    ...
    if (picos.length > 1)
      picos.sort((a, b) => (a.rssi < b.rssi ? 1 : a.rssi > b.rssi ? -1 : 0));
  ...
  */
    ...
    if (peripheral) {
      if (peripheral.connected) {
        ...
        bleManager.connectToDevice(peripheral.id).then((device) => {
          ...
          if (p.connected) {
            this.readData(device, peripheral).then(
  ...
  async getSerial(peripheral) {
    bleManager
      ...
      .then((value) => {
        // 이게 어떤 원리로 작동하는지는 모르겠습니다.
        let val = Buffer.Buffer.from(value.value, 'base64');
        DeviceInfo.serial = val;
        // let serial = base64.decode(value.value);
        // DeviceInfo.data = Object.assign({}, DeviceInfo, data, {serial: serial});
      })
      .then(() => this.setState({isConnected: 1}));
  }
}
...
const DeviceInfo = {
  ...
  reload: function () {
    if (DeviceInfo.device == null)
      DeviceInfo.device = bleManager.connectedDevices([])[0];
    let serviceUUID = '0000ffb0-0000-1000-8000-00805f9b34fb';
    let characteristicUUID = '0000ffb3-0000-1000-8000-00805f9b34fb';
